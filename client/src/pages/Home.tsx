...
function RStreakScreen({ goBack, navigate, notify }: { goBack: () => void; navigate: (screen: Screen) => void; notify: (message: string) => void }) {
  ...
  return (
    <>
      <AppHeader title={<>R-Streak <span className="title-fire">🔥</span></>} onBack={goBack} right={<button className="round-control" aria-label="How streaks work" onClick={() => navigate("help")}><CircleHelp size={21} /></button>} />
      <div className="app-scroll r-streak-scroll">
        <button className="reconcile-banner" onClick={() => navigate("reconcile")}><span className="target-emoji">🎯</span><span><b>Reconcile Today’s Accounts</b><small>Stay consistent. Build your streak.</small></span><ChevronRight size={26} /></button>
...
function TransactionsScreenV2({ goBack, navigate, notify }: { goBack: () => void; navigate: (screen: Screen) => void; notify: (message: string) => void }) {
  ...
  const [rows, setRows] = useState<Row[]>([
    ...
    { id: "salary", name: "Salary", category: "Income · HDFC Bank", amount: 45000, date: "Today", categorised: true, icon: ArrowDownLeft, tone: "mint" },
    { id: "amazon", name: "Amazon", category: "Shopping", amount: -1299, date: "Yesterday", categorised: true, icon: ShoppingBag, tone: "lilac" },
  ]);
  ...
  const [draftAmount, setDraftAmount] = useState("");
  const [draftCategory, setDraftCategory] = useState("Uncategorised");
  const shown = rows.filter((row) => (group === "Categorised") === row.categorised).filter((row) => amountFilter === "All" || (amountFilter === "Large" ? Math.abs(row.amount) >= 1000 : Math.abs(row.amount) < 1000)).sort((a, b) => sort === "Highest" ? Math.abs(b.amount) - Math.abs(a.amount) : 0);
  ...
  const save = () => {
    ...
    if (!draftName.trim() || !amount) { notify("Add a merchant and amount to continue"); return; }
    const categorised = draftCategory !== "Uncategorised";
    if (formMode === "edit" && selected) setRows((current) => current.map((row) => row.id === selected.id ? { ...row, name: draftName.trim(), amount: -amount, category: categorised ? draftCategory : "Needs review", categorised, tone: categorised ? "gold" : "coral" } : row));
    else setRows((current) => [{ id: `entry-${Date.now()}`, name: draftName.trim(), amount: -amount, category: categorised ? draftCategory : "Needs review", date: "Today", categorised, icon: ReceiptText, tone: categorised ? "gold" : "coral" }, ...current]);
    setGroup(categorised ? "Categorised" : "Uncategorised"); setFormMode(null); setSelected(null); notify(formMode === "edit" ? "Transaction updated" : "Transaction added");
  };
  return <><AppHeader title="Transactions" subtitle="Track every move with clarity." onBack={goBack} right={<button className="round-control" aria-label="Add transaction" onClick={openAdd}><Plus size={22} /></button>} /><div className="app-scroll transaction-screen"><div className="balance-strip"><span>Available to spend</span><b>₹11,550</b><small><TrendingUp size={13} /> ₹2,120 more than last week</small></div><div className="filter-row"><button className="filter-more"><CalendarDays size={16} /><select aria-label="Transaction period" value={period} onChange={(event) => setPeriod(event.target.value)}><option>Aug 2026</option><option>Jul 2026</option><option>This year</option></select></button></div><section className="transaction-list card"><div className="list-header"><h2>Transactions</h2><button className="text-link" onClick={openAdd}>Add <Plus size={14} /></button></div><div className="transaction-category-row" role="tablist" aria-label="Transaction category"><button className={`transaction-category-card uncat ${group === "Uncategorised" ? "active" : ""}`} onClick={() => setGroup("Uncategorised")}><span><ReceiptText size={17} /><b>Uncategorised</b></span><strong>{rows.filter((row) => !row.categorised).length}</strong><small>Needs review</small></button><button className={`transaction-category-card cat ${group === "Categorised" ? "active" : ""}`} onClick={() => setGroup("Categorised")}><span><Check size={17} /><b>Categorised</b></span><strong>{rows.filter((row) => row.categorised).length}</strong><small>Organised activity</small></button></div><div className="transaction-tools persistent-tools"><label className="tool-chip">Sort:<select aria-label="Transaction sorting" value={sort} onChange={(event) => setSort(event.target.value as "Newest" | "Highest")}><option>Newest</option><option>Highest</option></select></label><label className="tool-chip">Filter:<select aria-label="Transaction amount filter" value={amountFilter} onChange={(event) => setAmountFilter(event.target.value as "All" | "Large" | "Small")}><option>All</option><option>Large</option><option>Small</option></select></label><span>{shown.length} shown</span></div><div className="transaction-tab-panel" role="tabpanel">{group === "Uncategorised" && shown.length ? <div className="tab-panel-note"><span><b>Review your transactions</b><small>Apply a sensible category to all pending records.</small></span><button onClick={() => { setRows((current) => current.map((row) => row.categorised ? row : { ...row, category: "Food & Dining", categorised: true, tone: "gold" })); setGroup("Categorised"); notify("Pending transactions categorised"); }}>Review all</button></div> : null}{shown.map((row) => <button className="full-transaction" key={row.id} onClick={() => setSelected(row)}><span className="transaction-date">{row.date}</span><MiniIcon icon={row.icon} tone={row.tone} /><span className="transaction-name"><b>{row.name}</b><small>{row.category}</small></span><strong className={row.amount > 0 ? "positive" : ""}>{row.amount > 0 ? "+" : "-"}₹{Math.abs(row.amount).toLocaleString("en-IN")}</strong><ChevronRight size={16} /></button>)}{!shown.length ? <div className="transaction-empty"><SlidersHorizontal size={19} /><b>No {group.toLowerCase()} transactions</b><small>Try a different filter or add a transaction.</small></div> : null}</div></section><button className="transaction-insight" onClick={() => navigate("insight-detail")}><Lightbulb size={20} /><span><b>You spent ₹3,040 more this month.</b><small>See the three categories driving the change.</small></span><ChevronRight size={18} /></button></div>{selected && !formMode ? <div className="transaction-sheet-layer" role="dialog" aria-modal="true" aria-label="Transaction details"><button className="sheet-scrim" aria-label="Close details" onClick={() => setSelected(null)} /><section className="transaction-sheet"><button className="sheet-close" aria-label="Close details" onClick={() => setSelected(null)}><X size={18} /></button><MiniIcon icon={selected.icon} tone={selected.tone} /><small>{selected.date} · {selected.category}</small><h2>{selected.name}</h2><strong className={selected.amount > 0 ? "positive" : ""}>{selected.amount > 0 ? "+" : "-"}₹{Math.abs(selected.amount).toLocaleString("en-IN")}</strong><div className="sheet-meta"><span>Account <b>HDFC Bank</b></span><span>Status <b>{selected.categorised ? "Categorised" : "Needs review"}</b></span></div><button className="sheet-primary" onClick={() => { setDraftName(selected.name); setDraftAmount(String(Math.abs(selected.amount))); setDraftCategory(selected.categorised ? selected.category : "Uncategorised"); setFormMode("edit"); }}>Edit transaction</button><button className="sheet-danger" onClick={() => { setRows((current) => current.filter((row) => row.id !== selected.id)); setSelected(null); notify(`${selected.name} deleted`); }}><Trash2 size={16} /> Delete transaction</button></section></div> : null}{formMode ? <div className="transaction-sheet-layer" role="dialog" aria-modal="true" aria-label={formMode === "edit" ? "Edit transaction" : "Add transaction"}><button className="sheet-scrim" aria-label="Close transaction form" onClick={() => { setFormMode(null); setSelected(null); }} /><form className="transaction-sheet add-sheet" onSubmit={(event) => { event.preventDefault(); save(); }}><button type="button" className="sheet-close" aria-label="Close transaction form" onClick={() => { setFormMode(null); setSelected(null); }}><X size={18} /></button><SmallLabel>{formMode === "edit" ? "Edit transaction" : "New transaction"}</SmallLabel><h2>{formMode === "edit" ? "Edit expense" : "Add an expense"}</h2><label>Merchant<input autoFocus value={draftName} onChange={(event) => setDraftName(event.target.value)} placeholder="e.g. Coffee shop" /></label><label>Amount<input inputMode="decimal" value={draftAmount} onChange={(event) => setDraftAmount(event.target.value)} placeholder="0" /></label><label>Category<select value={draftCategory} onChange={(event) => setDraftCategory(event.target.value)}><option>Uncategorised</option><option>Food & Dining</option><option>Shopping</option><option>Transport</option><option>Entertainment</option></select></label><button className="sheet-primary" type="submit">{formMode === "edit" ? "Save transaction" : <><Plus size={16} /> Add transaction</>}</button></form></div> : null}</>;
}

function InsightsScreen({ notify, navigate }: { notify: (message: string) => void; navigate: (screen: Screen) => void }) {
  ...
  return (
    <>
      <AppHeader title={<><Sparkles size={23} className="heading-sparkle" /> Insights</>} subtitle="Understand. Improve. Win." right={<button className="date-selector" onClick={() => navigate("reports")}><CalendarDays size={17} /> Aug 2026 <ChevronDown size={15} /></button>} />
      <div className="app-scroll insight-screen">
        <section className="card budget-card">
          <div className="section-heading"><h2>Budgets</h2><button className="text-link" onClick={() => navigate("budgets")}>View more <ArrowRight size={16} /></button></div>
          ...
          <div className="budget-lines">{budgets.map(([label, sub, value, color, caption, Icon]) => <button className="budget-line" key={String(label)} onClick={() => navigate("budget-detail")}><MiniIcon icon={Icon as LucideIcon} tone={color === "coral" ? "coral" : color === "gold" ? "gold" : color === "purple" ? "lilac" : color === "blue" ? "blue" : "mint"} /><span><b>{label}</b><small>{sub}</small><ProgressBar value={Number(value)} color={color as "green" | "coral" | "gold" | "purple" | "blue"} thin /></span><strong className={Number(value) > 90 ? "negative" : ""}>{value}%<em>{caption}</em></strong><ChevronRight size={15} /></button>)}</div>
          <button className="budget-all-link" onClick={() => navigate("budgets")}>View all categories <ArrowRight size={16} /></button>
        </section>
        <section className="card alert-card"><div className="section-heading"><h2>Budget Alerts</h2><button className="text-link" onClick={() => navigate("insight-detail")}>View all <ArrowRight size={15} /></button></div><div className="alert-row"><span>🍴 <b>Food is over budget</b><em>by ₹420</em></span><span>👜 <b>Shopping is over budget</b><em>by ₹200</em></span><span>📄 <b>Bills & Utilities</b><em>70% used</em></span></div></section>
        <section className="goals-section"><div className="section-heading"><h2>Goals</h2><button className="text-link" onClick={() => navigate("goals")}>View all <ArrowRight size={15} /></button></div><div className="goal-cards">{([[ShieldCheck, "Emergency Fund", "68,000", "1,00,000", 68, "green"], [Banknote, "MacBook Fund", "72,000", "1,20,000", 60, "gold"], [Goal, "Goa Trip", "24,000", "50,000", 48, "purple"]] as Array<[LucideIcon, string, string, string, number, "green" | "gold" | "purple"]>).map(([Icon, title, amount, target, progress, color]) => <button className="card goal-card" key={title} onClick={() => navigate("goal-detail")}><MiniIcon icon={Icon} tone={color === "gold" ? "gold" : color === "purple" ? "lilac" : "mint"} /><b>{title}</b><strong>₹{amount}</strong><span>of ₹{target}</span><ProgressBar value={progress} color={color} thin /><small>{progress}%</small></button>)}</div></section>
        <section className="card spend-analysis"><div className="section-heading"><h2>Spending Analysis <Info size={14} /></h2><span>vs last month</span></div><div className="analysis-body"><div><span>Total Spending</span><b>₹28,450</b><small className="negative">▲ 12%</small><p>You spent <strong>₹3,040</strong> more than last month.</p></div><div>{budgets.map(([label, , value, color]) => <div className="analysis-line" key={String(label)}><span>{label}</span><ProgressBar value={Number(value)} color={color as "green" | "coral" | "gold" | "purple" | "blue"} thin /><b className={color === "green" ? "positive" : "negative"}>{color === "green" ? "▼ 8%" : "▲"}</b></div>)}</div></div><button className="pale-callout" onClick={() => navigate("insight-detail")}><Lightbulb size={15} /> Your food spending is 34% higher than last month. <ChevronRight size={15} /></button></section>
        <section className="moves-card"><div><h2>Your Top 3 Moves <Info size={14} /></h2>{[["01", "Reduce food spending", "₹2,400"], ["02", "Cancel unused subscriptions", "₹1,847"], ["03", "Move ₹5,000 to savings on payday", "₹3,000"]].map(([num, task, saving]) => <button key={num} onClick={() => navigate("money-saving")}><span>{num}</span><b>{task}</b><em>{saving} /mo</em></button>)}</div><aside><span>Potential monthly improvement</span><b>₹7,240</b><button onClick={() => navigate("money-saving")}>Take action <ChevronRight size={17} /></button></aside></section>
      </div>
    </>
  );
}
...
function MoreScreen({ navigate, notify }: { navigate: (screen: Screen) => void; notify: (message: string) => void }) {
  const management: Array<[LucideIcon, string, string, Screen]> = [[ReceiptText, "Tax Returns", "File & manage", "tax-returns"], [PiggyBank, "Money Saving", "Track savings", "money-saving"], [WalletCards, "Documents", "Store & organize", "documents"], [Landmark, "Net Worth", "Track overall value", "net-worth"], [LineChart, "Investments", "Track portfolio", "investments"], [Goal, "Goals", "Track your goals", "goals"], [Goal, "Budgets", "Manage budgets", "budgets"], [RefreshCw, "Recurring", "Manage bills", "recurring"], [Goal, "Categories", "Manage spending labels", "categories"], [Target, "Tags", "Manage custom labels", "tags"]];
  return (
    <>
      <AppHeader title="More" subtitle="Everything you need. All in one place." right={<button className="round-control notification-dot" aria-label="Open notifications" onClick={() => navigate("notifications")}><Bell size={20} /></button>} />
      <div className="app-scroll more-screen">
        <button className="profile-card" style={{ backgroundImage: `linear-gradient(105deg, rgba(7,72,33,.98), rgba(11,99,47,.88)), url(${FOREST_TEXTURE})` }} onClick={() => navigate("profile")}><span className="profile-orb">P<Check size={15} /></span><span><b>Prudhvi Raj</b><small>Financial Score <Info size={13} /></small><strong>82 <em>/100</em></strong><i><TrendingUp size={15} /> Great job! Keep it up.</i></span><svg viewBox="0 0 170 50" aria-hidden="true"><path d="M0 44 C18 35 32 40 46 33 S76 39 94 25 S118 31 134 15 S154 15 170 0" /></svg><ChevronRight size={23} /></button>
        <SmallLabel>Quick actions</SmallLabel><div className="quick-actions">{([[LineChart, "Reports", "See insights", "mint", "reports"], [ArrowRight, "Split", "Expenses", "mint", "transactions"], [Settings, "Settings", "Manage app", "lilac", "settings"], [CircleHelp, "Help & Support", "Get help", "coral", "help"]] as Array<[LucideIcon, string, string, "mint" | "lilac" | "coral", Screen]>).map(([Icon, title, sub, tone, destination]) => <button key={title} onClick={() => navigate(destination)}><MiniIcon icon={Icon} tone={tone} /><b>{title}</b><small>{sub}</small></button>)}</div>
        <SmallLabel>Manage finances</SmallLabel><div className="manage-grid">{management.map(([Icon, title, sub, destination]) => <button key={title} onClick={() => navigate(destination)}><MiniIcon icon={Icon} tone="mint" /><b>{title}</b><small>{sub}</small><ChevronRight size={16} /></button>)}</div>
        <SmallLabel>More tools</SmallLabel><section className="card tools-card">{([[ShieldCheck, "Security & Privacy", "Manage security, passcode, data & more", "mint", "security"], [Banknote, "Backup & Restore", "Backup your data and restore anytime", "blue", "backup-restore"], [Info, "About Finella", "App info, terms & policies", "lilac", "about"]] as Array<[LucideIcon, string, string, "mint" | "blue" | "lilac", Screen]>).map(([Icon, title, sub, tone, destination]) => <button key={title} onClick={() => navigate(destination)}><MiniIcon icon={Icon} tone={tone} /><span><b>{title}</b><small>{sub}</small></span><ChevronRight size={17} /></button>)}</section>
      </div>
    </>
  );
}
  ...
  return (
    <>
      ...
      <div className="app-scroll settings-screen">
        ...
        <div className="settings-columns"><div><h2 className="settings-label">Data & Sync <span><ShieldCheck size={13} /> Your data is private & secure</span></h2><section className="card settings-list compact">{dataItems.map(([Icon, title, sub, action]) => <button key={title} onClick={() => { const destination: Partial<Record<string, Screen>> = { "SMS Parsing": "documents", "Bank Sync": "bank-accounts", "Email Parsing": "documents", "Auto-Transactions": "transactions", "PDF Statement": "documents" }; if (action === "toggle") setManualEntry(!manualEntry); else if (destination[title]) navigate(destination[title]); }}><MiniIcon icon={Icon} tone="mint" /><span><b>{title}</b><small>{sub}</small></span>{action === "toggle" ? <Toggle active={manualEntry} onClick={() => setManualEntry(!manualEntry)} label="Toggle manual entry" /> : action ? <em>{action}</em> : null}{action !== "toggle" ? <ChevronRight size={16} /> : null}</button>)}</section></div>
          <div><h2 className="settings-label">AI Preferences</h2><section className="card settings-list compact"><button onClick={() => setWhatsapp(!whatsapp)}><MiniIcon icon={Send} tone="mint" /><span><b>WhatsApp Notifications</b><small>Receive alerts on WhatsApp</small></span><Toggle active={whatsapp} onClick={() => setWhatsapp(!whatsapp)} label="Toggle WhatsApp notifications" /></button><button onClick={() => setPush(!push)}><MiniIcon icon={Bell} tone="mint" /><span><b>Push Notifications</b><small>Get notified instantly</small></span><Toggle active={push} onClick={() => setPush(!push)} label="Toggle push notifications" /></button><button onClick={() => navigate("notifications")}><MiniIcon icon={RefreshCw} tone="mint" /><span><b>AI Alert Frequency</b><small>How often you get alerts</small></span><em>Real-time</em><ChevronRight size={16} /></button></section><h2 className="settings-label">Security</h2><section className="card settings-list compact"><button onClick={() => navigate("security")}><MiniIcon icon={LockKeyhole} tone="mint" /><span><b>Reset PIN</b><small>Change your app PIN</small></span><ChevronRight size={16} /></button><button onClick={() => navigate("security")}><MiniIcon icon={ScanLine} tone="mint" /><span><b>Biometric Unlock</b><small>Use fingerprint/face ID</small></span><em>Enabled</em><ChevronRight size={16} /></button></section></div></div>
        <div className="settings-bottom"><section className="card settings-list compact"><button onClick={() => navigate("profile")}><MiniIcon icon={Banknote} tone="mint" /><span><b>Base Currency</b><small>INR (₹)</small></span><ChevronRight size={15} /></button><button onClick={() => setDarkMode(!darkMode)}><MiniIcon icon={Moon} tone="mint" /><span><b>Dark Mode</b><small>Use dark theme</small></span><Toggle active={darkMode} onClick={() => setDarkMode(!darkMode)} label="Toggle dark mode" /></button></section><section className="card settings-list compact"><button onClick={() => navigate("about")}><MiniIcon icon={Info} tone="mint" /><span><b>Finella</b><small>Version 1.0.0</small></span><ChevronRight size={15} /></button></section></div>
...
function BudgetsScreen({ goBack, navigate, notify }: { goBack: () => void; navigate: (screen: Screen) => void; notify: (message: string) => void }) {
  ...
  return <><AppHeader title="Budgets" subtitle="Give every rupee a clear job." onBack={goBack} right={<button className="round-control" aria-label="Add budget" onClick={() => setShowForm(true)}><Plus size={21} /></button>} /><div className="app-scroll feature-screen"><section className="feature-hero budget-hero"><SmallLabel>August 2026</SmallLabel><div><span>Total planned</span><b>₹{limit.toLocaleString("en-IN")}</b><em>₹{(limit - spent).toLocaleString("en-IN")} still available</em></div><ProgressBar value={Math.round((spent / limit) * 100)} /><p><CheckCircle2 size={16} /> {Math.round((spent / limit) * 100)}% allocated this month</p></section><section className="card feature-card"><div className="section-heading"><div><SmallLabel>Spending plan</SmallLabel><h2>Budget breakdown</h2></div><button className="text-link" onClick={() => setShowForm(true)}>Add budget <Plus size={15} /></button></div><div className="segmented-control"><button className={kind === "category" ? "active" : ""} onClick={() => setKind("category")}><Goal size={15} /> By Category</button><button className={kind === "tag" ? "active" : ""} onClick={() => setKind("tag")}><Target size={15} /> By Tag</button></div><div className="feature-list">{budgets.map((budget) => <BudgetCardRow budget={budget} key={budget.name} onClick={() => navigate("budget-detail")} />)}</div></section><section className="card action-note"><CircleAlert size={19} /><span><b>One budget needs attention</b><small>Food & Dining is ₹420 over the plan.</small></span><button onClick={() => navigate("insight-detail")}>Review <ChevronRight size={15} /></button></section></div>{showForm ? <div className="transaction-sheet-layer" role="dialog" aria-modal="true" aria-label="Add budget"><button className="sheet-scrim" aria-label="Close add budget" onClick={() => setShowForm(false)} /><form className="transaction-sheet add-sheet" onSubmit={(event) => { event.preventDefault(); if (!draftName.trim() || !Number(draftLimit)) { notify("Add a budget name and amount to continue"); return; } setShowForm(false); setDraftName(""); setDraftLimit(""); notify("Budget created for August"); }}><button type="button" className="sheet-close" aria-label="Close add budget" onClick={() => setShowForm(false)}><X size={18} /></button><SmallLabel>New plan</SmallLabel><h2>Add a budget</h2><label>Budget name<input autoFocus value={draftName} onChange={(event) => setDraftName(event.target.value)} placeholder="e.g. Travel" /></label><label>Monthly limit<input inputMode="decimal" value={draftLimit} onChange={(event) => setDraftLimit(event.target.value)} placeholder="0" /></label><label>Budget type<select value={kind} onChange={(event) => setKind(event.target.value as BudgetKind)}><option value="category">Category</option><option value="tag">Tag</option></select></label><button className="sheet-primary" type="submit"><Plus size={16} /> Create budget</button></form></div> : null}</>;
}
...
function ReportsScreen({ goBack, navigate }: { goBack: () => void; navigate: (screen: Screen) => void }) {
  ...
  return <><AppHeader title="Reports" subtitle="See the structure behind your money." onBack={goBack} right={<div className="transaction-tool-wrap"><button className="date-selector" onClick={() => setShowPeriods(!showPeriods)}><CalendarDays size={16} /> {period} <ChevronDown size={14} /></button>{showPeriods ? <div className="tool-menu report-period-menu">{["August 2026", "July 2026", "This quarter"].map((value) => <button className={period === value ? "active" : ""} key={value} onClick={() => { setPeriod(value); setShowPeriods(false); }}>{value}</button>)}</div> : null}</div>} /><div className="app-scroll feature-screen"><div className="segmented-control report-tabs">{(["Position", "P&L", "Cash flow"] as const).map((item) => <button className={tab === item ? "active" : ""} onClick={() => setTab(item)} key={item}>{item}</button>)}</div>{tab === "Position" ? <><section className="feature-hero report-hero"><SmallLabel>Financial position</SmallLabel><b>₹2,48,500</b><span>Liquid net worth</span><p><TrendingUp size={16} /> ₹12,450 higher than last month</p></section><section className="card feature-card"><h2>Assets and liabilities</h2>{[["Bank accounts", "2,85,000", "bank-accounts"], ["Investments", "48,500", "investments"], ["Credit cards", "-18,500", "credit-cards"], ["Loans", "-18,000", "loans"]].map(([label, amount, destination]) => <button className="feature-row" key={label} onClick={() => navigate(destination as Screen)}><span><b>{label}</b><small>Updated today</small></span><strong className={amount.startsWith("-") ? "negative" : ""}>₹{amount}</strong><ChevronRight size={16} /></button>)}</section></> : null}{tab === "P&L" ? <><section className="card report-stat-grid"><div><span>Income</span><b className="positive">₹45,000</b></div><div><span>Expenses</span><b className="negative">₹28,450</b></div><div><span>Net saved</span><b>₹16,550</b></div></section><section className="card feature-card"><h2>Expense drivers</h2>{categoryBudgets.slice(0, 3).map((budget) => <button className="feature-row" key={budget.name} onClick={() => navigate("budget-detail")}><span><b>{budget.name}</b><small>August spending</small></span><strong>₹{budget.spent.toLocaleString("en-IN")}</strong><ChevronRight size={16} /></button>)}</section></> : null}{tab === "Cash flow" ? <><section className="card report-stat-grid"><div><span>Inflow</span><b className="positive">₹47,250</b></div><div><span>Outflow</span><b className="negative">₹30,500</b></div><div><span>Closing</span><b>₹2,85,000</b></div></section><section className="card feature-card"><h2>Cash movement</h2><div className="cashflow-bars"><span style={{ height: "55%" }} /><span style={{ height: "78%" }} /><span style={{ height: "46%" }} /><span style={{ height: "92%" }} /><span style={{ height: "61%" }} /></div><button className="sheet-primary" onClick={() => navigate("transactions")}>Inspect transactions <ChevronRight size={16} /></button></section></> : null}</div></>;
}
